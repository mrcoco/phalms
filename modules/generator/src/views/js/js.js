$(document).ready(function(){
    var url_path = "{module_url}";
    var {module_name_l}_grid = $("#grid-{module_name_l}").bootgrid({
        ajax: true,
        url: url_path+"list",
        selection: true,
        multiSelect: true,
        templates: {
            header:"<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\"><div class=\"{{css.search}}\"></div></div><div class=\"col-sm-6\"><div class=\"{{css.actions}}\"></div> <div class='btn btn-primary' id='create' class='command-add'> <span class=\"fa fa-plus-square-o\"></span> New {module_name}</div></div></div></div>",
        },
        formatters: {
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $(this).find(".command-edit").on("click", function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    $("#grid-{module_name_l}").bootgrid("reload");
                    setTimeout(function(){
                        $('#myModal').modal('hide')
                    }, 10000);
                }
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    {module_name_l}_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#my{module_name_l}').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myForm(status,e) {
        ...
        if(status == 'edit') {
            ...
            $('#my{module_name_l} .modal-title').html('Edit {module_name_l} '+e.data("row-id"));
            $.getJSON(url_path+"get/?id=" + e.data("row-id"), function (data) {
                $('#hidden_id').val(data.id);
                {js_fields}
            });
        }else{
            $('#my{module_name_l} .modal-title').html('Create New {module_name_l} ');
            
        }

        $('#my{module_name_l}').modal('show');
    ...
    function myAlert(e)
    {
        ...
        mesg["alert"] = e.alert;
        mesg["title"] = e.msg;
        mesg["msg"] = "#{module_name_l} "+e._id+" "+e.msg;
        notif_show(mesg);
    }
