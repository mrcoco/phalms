$(document).ready(function(){
    var url_path = "http://phalms.dev/sekolahan/";
    var sekolahan_grid = $("#grid-sekolahan").bootgrid({
        ajax: true,
        url: url_path+"list",
        selection: true,
        multiSelect: true,
        templates: {
            header:"<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\"><div class=\"{{css.search}}\"></div></div><div class=\"col-sm-6\"><div class=\"{{css.actions}}\"></div> <div class='btn btn-primary' id='create' class='command-add'> <span class=\"fa fa-plus-square-o\"></span> New Sekolahan</div></div></div></div>",
        },
        formatters: {
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $(this).find(".command-edit").on("click", function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    $("#grid-sekolahan").bootgrid("reload");
                    setTimeout(function(){
                        $('#myModal').modal('hide')
                    }, 10000);
                }
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    sekolahan_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#mysekolahan').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myForm(status,e) {
        ...
        if(status == 'edit') {
            ...
            $('#mysekolahan .modal-title').html('Edit sekolahan '+e.data("row-id"));
            ...
            $('#mysekolahan .modal-title').html('Create New sekolahan ');
            
        }

        $('#mysekolahan').modal('show');
    ...
    function myAlert(e)
    {
        ...
        mesg["alert"] = e.alert;
        mesg["title"] = e.msg;
        mesg["msg"] = "#sekolahan "+e._id+" "+e.msg;
        notif_show(mesg);
    }
