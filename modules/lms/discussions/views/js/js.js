$(document).ready(function(){
    var url_path = "http://phalms.dev/discussions/";
    var discussions_grid = $("#grid-discussions").bootgrid({
        ajax: true,
        url: url_path+"list",
        selection: true,
        multiSelect: true,
        templates: {
            header:"<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\"><div class=\"{{css.search}}\"></div></div><div class=\"col-sm-6\"><div class=\"{{css.actions}}\"></div> <div class='btn btn-primary' id='create' class='command-add'> <span class=\"fa fa-plus-square-o\"></span> New Discussions</div></div></div></div>",
        },
        formatters: {
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $(this).find(".command-edit").on("click", function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    $("#grid-discussions").bootgrid("reload");
                    setTimeout(function(){
                        $('#myModal').modal('hide')
                    }, 10000);
                }
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    discussions_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#mydiscussions').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myForm(status,e) {
        ...
        if(status == 'edit') {
            ...
            $('#mydiscussions .modal-title').html('Edit discussions '+e.data("row-id"));
            $.getJSON(url_path+"get/?id=" + e.data("row-id"), function (data) {
                $('#hidden_id').val(data.id);
                 $('#parrent_id').val(data.parrent_id);
	 ...
	 $('#content').val(data.content);
            ...
            $('#mydiscussions .modal-title').html('Create New discussions ');
            
        }

        $('#mydiscussions').modal('show');
    ...
    function myAlert(e)
    {
        ...
        mesg["alert"] = e.alert;
        mesg["title"] = e.msg;
        mesg["msg"] = "#discussions "+e._id+" "+e.msg;
        notif_show(mesg);
    }
