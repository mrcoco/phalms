$(document).ready(function(){
    var url_path = "http://phalms.dev/testing/";
    var testing_grid = $("#grid-testing").bootgrid({
        ajax: true,
        url: url_path+"list",
        selection: true,
        multiSelect: true,
        templates: {
            header:"<div id=\"{{ctx.id}}\" class=\"{{css.header}}\"><div class=\"row\"><div class=\"col-sm-6 actionBar\"><div class=\"{{css.search}}\"></div></div><div class=\"col-sm-6\"><div class=\"{{css.actions}}\"></div> <div class='btn btn-primary' id='create' class='command-add'> <span class=\"fa fa-plus-square-o\"></span> New Testing</div></div></div></div>",
        },
        formatters: {
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    testing_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#mytesting').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myForm(status,e) {
        ...
        if(status == 'edit') {
            ...
            $('#mytesting .modal-title').html('Edit testing '+e.data("row-id"));
	 ...
	 $('#kelas').val(data.kelas);
            ...
            $('#mytesting .modal-title').html('Create New testing ');
            
        }

        $('#mytesting').modal('show');
